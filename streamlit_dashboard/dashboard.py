...
def run_monte_carlo_simulation(daily_effort_df: pd.DataFrame, current_totals: pd.Series, days_remaining: int) -> pd.DataFrame:
    ...
    for team in teams:
        ...
        sim_sigma = np.sqrt(days_remaining * (sigma ** 2) + (days_remaining ** 2) * ((sigma ** 2) / n_days))
...
def render_standings_table(data: pd.DataFrame, columns: list, format_dict: dict = None) -> None:
...
with col_team:
    ...
    with tab_month:
        ...
        if not month_data.empty:
            stats = (month_data.groupby('Team')[['Effort', 'Distance (km)']]
                    .sum().sort_values('Effort', ascending=False).reset_index())
            st.dataframe(stats, width='stretch', hide_index=True)
        else:
            st.info("No activities for the current month.")

    with tab_year:
        stats = (filtered_df.groupby('Team')[['Effort', 'Distance (km)']]
                .sum().sort_values('Effort', ascending=False).reset_index())
        st.dataframe(stats, width='stretch', hide_index=True)
...
with col_indiv:
    ...
    indiv_format = {"Effort": "{:.1f}", "Distance (km)": "{:.1f}", "Time (min)": "{:.0f}"}

    with indiv_tab_month:
        st.caption(f"Standings for {TODAY.strftime('%B %Y')}")
        month_data_indiv = filtered_df[filtered_df['Month'] == CURRENT_MONTH]
        if not month_data_indiv.empty:
            stats = (month_data_indiv.groupby(['Name', 'Team'])[['Effort', 'Distance (km)', 'Time (min)']]
                    .sum().reset_index().sort_values('Effort', ascending=False).reset_index(drop=True))
            stats.index += 1
    ...
    with indiv_tab_year:
        stats = (filtered_df.groupby(['Name', 'Team'])[['Effort', 'Distance (km)', 'Time (min)']]
                .sum().reset_index().sort_values('Effort', ascending=False).reset_index(drop=True))
        stats.index += 1
        st.dataframe(stats.style.format(indiv_format), width='stretch')
...
st.dataframe(
    filtered_df.sort_values('Date', ascending=False)[DISPLAY_COLS].style.format(ACTIVITY_FORMAT),
    width='stretch',
    hide_index=True
)
